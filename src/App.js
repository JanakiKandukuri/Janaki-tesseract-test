...
import TesseractOcr, { LANG_ENGLISH } from "tesseract.js";
...
export default function ImagePickerExample() {
  ...
  const { TesseractWorker } = TesseractOcr;
  ...
  const extractText = async (imgPath) => {
    try {
      const tessOptions = {};
      await this.TesseractOcr.recognize(imgPath, "LANG_ENGLISH", tessOptions);
      setText(imgPath);
    } catch (err) {
      console.error(err);
    }
  ...
  const recognizeTextFromImage = async (path) => {
    try {
      ...
      await this.TesseractOcr.recognize(path, LANG_ENGLISH, tesseractOptions);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.options}>
        <View style={styles.button}>
          <Button disabled={isLoading} title="Picker" onPress={pickImage} />
        </View>
      </View>
      {imgSrc && (
        <View style={styles.imageContainer}>
          ...
          {isLoading ? (
            <Text showsText progress={progress} />
          ) : (
            <Text>{text}</Text>
          )}
        </View>
